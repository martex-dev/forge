...
import { applyDefaultLayout, applyInitialSizes, getLayoutApi } from './layout-controller';
...
export async function resetRoomLayout(room: RoomId): Promise<void> {
	try {
		...
		const panels = panelsFor(RENDERER_MODULES, enabled).filter((p) => p.room === room);
		applyDefaultLayout(api, panels);
		requestAnimationFrame(() => applyInitialSizes(api, panels));
